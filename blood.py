...
class pinfo(webapp2.RequestHandler):
	def get(self):
		...
		if user:
			...
			if result.count()>0:
				flag=1
	    		template_values={'flag':flag,'logout':logout,'result':result,'lngth':lngth,'request':request}
...
class prinfo(webapp2.RequestHandler):
	def get(self):
		...
		if user:
			...
			if result.count()>0:
				flag=1
                        template_values={'flag':flag,'result':result,'lngth':lngth,'request':request,'logout':logout}
...
class todaycamp(webapp2.RequestHandler):
	def get(self):
		...
		template_values={'lngth':lngth,'request':request,'final':json.dumps(final),'logout':logout}
...
class cinfo(webapp2.RequestHandler):
	def get(self):
		...
		if user:		  
	  		logout=users.create_logout_url("/")
                        template_values={'lngth':lngth,'request':request,'logout':logout}
...
class thank(webapp2.RequestHandler):
	def get(self):
		...
		for i in notify:
				request.append(i)
		lngth=len(request)
	    	template_values={'notify':notify,'lngth':lngth,'request':request,'logout':logout}
...
#to_addr = self.request.get("friend_email")	
			...
			message.to = to_addr
			message.body = """You can save many lives :\n"""+ rst[0].rname + """  needs  """ + rst[0].bunit  + """   units of  blood at """ + rst[0].address + """   please help him by donating the blood \n Contact No. """+ rst[0].contact
			message.send()
...
class view_mail(webapp2.RequestHandler):
	def get(self):
		...
		lngth=len(request)
	    	...
	    	template_values={'logout':logout,'request':request,'lngth':lngth}
...
class hero(webapp2.RequestHandler):
	def get(self):
		...
		lngth=len(request)
		logout=users.create_logout_url("/")
	    	template_values={'logout':logout,'posts':posts,'lngth':lngth,'request':request}
		...
		user=users.get_current_user()
	 	logout=users.create_logout_url("/")
	    	...
	    	for i in notify:
	 		request.append(i)
	    	lngth=len(request)
	    	template_values={'lngth':lngth,'request':request,'value':value,'p':p,'param1':param1,'logout':logout}
...
class camps_detail(webapp2.RequestHandler):
		...
		lngth=len(request)
	    	for i in result:
	    		fl=0
	    		for j in li:
	    			if str(i.key())==str(j.cid) and str(j.guser)==str(user.email()) and j.flag==1:
	    				having.append(i)
	    				fl=1
	    				break
	    		if fl==0:
	    			rest.append(i)		
                template_values={'having':having,'rest':rest,'user':user,'logout':logout,'lngth':lngth,'request':request}
...
class camp_post(webapp2.RequestHandler):
	def get(self,camp_id):
	    ...
	    template_values={'lngth':lngth,'request':request,'logout':logout,'post':post,'cuser':user,'cur_camp':cur_camp}
	...
	def get(self,get_key):	
		...
		for i in notify:
			if str(i.key())==str(get_key):
				...
				request.append(i)
		template_values={'request':request,'req':req,'lngth':lngth}
...
class show_donor(webapp2.RequestHandler):
	def get(self):
		...
		for i in notify:
				request.append(i)
		lngth=len(request)
		template_values={'request':request,'lngth':lngth,'locate':k[0].address,'vivek':a,'final':json.dumps(final),'long':float(r[1]),'latit':float(r[0]),'name':name,'a':a,'logout':logout,'q':q}
...
class going_event(webapp2.RequestHandler):
	    ...
	    lngth=len(request)
            logout=users.create_logout_url("/")
	    template_values={'request':request,'lngth':lngth,'user':user,'camp_user':camp_name,'address':ad,'start':fs,'end':fe,'logout':logout}
...
class coming_donors(webapp2.RequestHandler):
		...
		if fl==0:
			...
			while (tot%114)!=0:
					out+=" "
					tot+=1
	    template_values={'lngth':lngth,'donor':donr,'out':out,'logout':logout,'a':a,'request':request,'empty':empty}
...
class about(webapp2.RequestHandler):
	def get(self):
		...
		for i in notify:
				request.append(i)
		lngth=len(request)
	    	template_values={'logout':logout,'lngth':lngth,'request':request}
...
class organise(webapp2.RequestHandler):
	def get(self):
		...
		for i in camps:
				...
				if (q.count())>0:
					flag=1
                template_values={'logout':logout,'flag':flag,'camp_keys':camp_keys,'lngth':lngth,'request':request}
...
class about1(webapp2.RequestHandler):
	def get(self):
		...
		for i in notify:
				request.append(i)
		lngth=len(request)
		template_values={'login':login,'lngth':lngth,'request':request}
...
class share(webapp2.RequestHandler):
	def get(self):
		...
		lngth=len(request)
                ...
                feeds=story.all()
                template_values={'pstories':pstories,'feeds':feeds,'logout':logout,'lngth':lngth,'request':request}
	    	template=JINJA_ENVIRONMENT.get_template('html/share.html')
		self.response.write(template.render(template_values))		
class view_story(webapp2.RequestHandler):
	def get(self,story_key):
		...
		lngth=len(request)
                feeds=story.all()
		for i in feeds:
			if str(i.key())==str(story_key):
				ans=i
				break;
                template_values={'ans':ans,'ky':story_key,'logout':logout,'lngth':lngth,'request':request}
	    	template=JINJA_ENVIRONMENT.get_template('html/view_story.html')
		self.response.write(template.render(template_values))	
...
class stry(webapp2.RequestHandler):
	def get(self):
		...
		lngth=len(request)
		all_story=story.all()
                template_values={'stories':all_story,'logout':logout,'request':request,'lngth':lngth}
...
class all_notification(webapp2.RequestHandler):
	def get(self):
		user=users.get_current_user()
		notify=notification.all().filter("rid = ",str(user)).filter("flag = ",0)
		notific=notification.all().filter("rid = ",str(user))
	        cont=notific.count()
		request=[]
		for i in notify:
				k=i.key()
				...
				request.append(i)
		lngth=0
	    	template_values={'cont':cont,'notify':notify,'lngth':lngth,'notific':notific}
	    	template=JINJA_ENVIRONMENT.get_template('html/all_notification.html')
		self.response.write(template.render(template_values))		
class faq(webapp2.RequestHandler):
	def get(self):
		...
		for i in notify:
				request.append(i)
		lngth=len(request)
                template_values={'notify':notify,'lngth':lngth,'logout':logout}
